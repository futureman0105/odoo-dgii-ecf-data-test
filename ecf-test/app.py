...
def action_check_dgii_status(self):
    try:
        ...
        response = requests.get(
            f"{self.base_url}/consultaresultado/api/consultas/estado?trackid={__trackId}",
            headers=headers,
            timeout=30
        )
        ...
        if estado == "Aceptado":
            __logger.info(f"DGII Tracking Status: accepted")

        elif estado == "Rechazado":
            __logger.info(f"DGII Tracking Status: {estado}")
        else:
            __logger.info(f"DGII Tracking Status: accepted")

        __logger.info(f"DGII Message: {mensajes}")

    except Exception as e:
        __logger.error(f"DGII Tracking API error: {e}")
